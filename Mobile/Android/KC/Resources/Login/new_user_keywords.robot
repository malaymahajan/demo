...
${invalid_email_2}          user&@example.c
...
Enter Otp In Otp Fields For New User
    ...
    Input Text    ${enter_otp_field}    ${otp_data}
...
Check Otp Delivery
    ...
    IF    ${status}    Click Element    ${clear_notification}
    Open Notification Panel
    Wait Until Element Is Visible    ${otp_notification_text}    timeout=120
    Element Should Be Visible    ${otp_notification_text}    timeout=120
    Go Back
...
Verify Validation For Email Address
    ...
    Click Element    ${email}
    Enter Invalid Email Address    ${invalid_email_1}
    Swipe    ${630}    ${974}    ${636}    ${365}
    Wait Until Element Is Visible    ${Next_button}
    Click Element    ${Next_button}
    Wait Until Page Contains Element    ${Email_toast}    timeout=30
    Sleep    3
    Clear Text    ${email}
    Enter Invalid Email Address    ${invalid_email_2}
    Wait Until Element Is Visible    ${Next_button}
    Click Element    ${Next_button}
    Wait Until Page Contains Element    ${Email_toast}    timeout=30
    Sleep    3
...
Check Complete Profile PopUP
    ...
    ...    timeout=30
    IF    ${status}    Click Element    ${complete_profile_popup_okbtn}
...
Check All Matches Visible
    ...
    FOR    ${index}    IN RANGE    1    ${expected_match_count + 1}
        ${element}    Replace String    ${matchcenter_button}    index    ${index}
        Log    ${index}
        Log    ${matchcentre_btn}
        Log    ${element}
        ...
        Swipe Down Until Element Is Visible    xpath=${element}
    END
