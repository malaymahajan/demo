...
Verify Add To Calender Functionality Using Apple ID
    [Arguments]    ${email}
    Wait And Click On Element     xpath=${apple_text}
    wait until page contains element     xpath=${email_field_apple}       timeout=30
    Input Text    xpath=${email_field_apple}    ${email}
    Wait And Click On Element    xpath=${sync_kkr_matches_text}
    Wait And Click On Element     accessibility_id=${submit_id}
    Wait And Click On Element     xpath=${click_to_sync_link}
    Wait And Click On Element     accessibility_id=${subscribe_id}
    Wait And Click On Element     xpath=${add_calendar}
    Wait And Click On Element     xpath=${add_button}
