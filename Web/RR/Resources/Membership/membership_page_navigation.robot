...
Check The First And last Name Of Junior And Juniors Name In Mail
     &{data}    fetch data by id    ${testdata}    12
     ${body}=  Search And Fetch Email    ${data.imapaddress}    ${data.port}   ${data.emailid_mail}     ${data.app_password}    ${data.mail_title}
     Log   ${body}
     ${mailbody}  Convert To String    ${body}
     Log   ${mailbody}
     Should Contain   ${mailbody}    ${data.Juniorfirstname}    ${data.Juniorlastname}      ${data.JuniorFullname}     ${data.Juniorfullnametwo}
